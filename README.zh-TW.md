...
Quill 預設透過 Railway 上的 Quill Cloud 將請求轉送至 OpenAI；若切換為自帶 API Key，則直接連線到你設定的相容端點。

- **內容不留存。** Quill Cloud 不儲存截圖、選取文字、Prompt 或 AI 回覆。
- **最少量匿名指標。** 每日活躍、達到限額與升級意願會以伺服器 HMAC 後的安裝識別去重，原始事件最多保存 90 天。
